...
class WC_Facebookcommerce_Integration extends WC_Integration {
  ...
  public function __construct() {
    ...
    if (is_admin()) {
      ...
      add_action('wp_ajax_ajax_save_settings',
        array($this, 'ajax_save_settings'), self::FB_PRIORITY_MID);

      add_action('wp_ajax_ajax_delete_settings',
        array($this, 'ajax_delete_settings'));

      add_action('wp_ajax_ajax_sync_all_products',
        array($this, 'ajax_sync_all_products'));

      add_action('wp_ajax_ajax_reset_all_products',
        array($this, 'ajax_reset_all_products'));
      ...
      if ($this->api_key && $this->product_catalog_id) {
        ...
        add_action('trash_product', array($this, 'on_product_trash'), 10, 1);
  ...
  public function fb_render_product_columns($column) {
    ...
    if ($column === 'fb') {
      ...
      if (!$fb_product_group_id) {
        ...
        $data_tip = $checkbox_value === '' ? 'Staging': 'Published';
        printf('<span class="tips" id="tip_%1$s" data-tip="%2$s">
          <input id="viz_%1$s" title="Visible on Facebook Shop" name="%3$s"
          type="checkbox" value="1" %4$s
          onclick="fb_toggle_visibility(%1$s)"/></span>',
            $post->ID,
            $data_tip,
            self::FB_VISIBILITY,
  ...
  public function fb_product_meta_box_html() {
    ...
    $fb_product_group_id = get_post_meta(
      ...
      true);
    if ($fb_product_group_id) {
      echo 'Facebook ID: <a href="https://facebook.com/'.
          $fb_product_group_id . '" target="_blank">' .
          $fb_product_group_id . '</a><br/>
          Visible:  ';

      $checkbox_value = get_post_meta($post->ID, self::FB_VISIBILITY, true);

      printf('<input name="%1$s" type="checkbox" value="1" %2$s>',
        self::FB_VISIBILITY,
        $checkbox_value === '' ? '' : 'checked');
    } else {
      echo "<b>This product is not yet synced to Facebook.</b>";
    }
  }
  ...
  function on_product_trash($wp_id) {
  ...
  function prepare_product($woo_product, $retailer_id = null) {
    ...
    foreach ($attachment_ids as $attachment_id) {
      ...
      if (!empty($attachment_url)) {
        array_push($additional_image_urls, $attachment_url);
      }
    }
    ...
    $checkout_url = wc_get_cart_url() . '?add-to-cart=';

    if ($woo_product->get_type() === 'variation') {
      $checkout_url = $checkout_url . $woo_product->parent->get_id() .
        '&variation_id=' . $woo_product->get_id();

      foreach ($woo_product->get_variation_attributes() as $name => $value) {
        $checkout_url = $checkout_url . '&' . $name . '=' . $value;
      }

    } else {
      $checkout_url = $checkout_url . $woo_product->get_id();
    }
    ...
    // prices in cents
    $price = floatval($woo_product->get_regular_price()) * 100;
    $sale_price = floatval($woo_product->get_sale_price()) * 100;
    $product_data = array(
      'name' => $post_title,
  ...
  */
  ...
  function checks() {
    ...
    if (!$this->api_key || !$this->product_catalog_id) {
      echo $this->get_message_html(sprintf(__('%1$sFacebook for WooCommerce
        is almost ready.%2$s To get started, %3$scomplete the setup steps%4$s.',
  ...
  function reset_all_products() {
    ...
    foreach ($post_ids as $post_id) {
      delete_post_meta($post_id, self::FB_PRODUCT_GROUP_ID);
      delete_post_meta($post_id, self::FB_PRODUCT_ITEM_ID);
      delete_post_meta($post_id, self::FB_VISIBILITY);
    }

    self::log("Product FBIDs deleted");
  }

  function ajax_reset_all_products() {
    $this->reset_all_products();
    wp_reset_postdata();
    wp_die();
  }
   ...
   **/
  function ajax_sync_all_products() {
    if (!$this->api_key || !$this->product_catalog_id) {
    ...
    $this->fblog(
      'Attempting to sync ' . $total . ' products with settings: ',
      $sanitized_settings,
      false);
    ...
    foreach ($post_ids as $post_id) {
      // Repeatedly overwrite sync flag while in actual sync loop
        ...
        <div class="content">
          ...
          <?php
            ...
            if (!current_user_can('manage_woocommerce')) {
              ...
              $currently_sycing = get_transient(self::FB_SYNC_IN_PROGRESS);
              // If a page name exists, setup is complete.
              if ('' != $page_name) {

                echo '<p id="connection_status">' .
                sprintf(__('Currently connected to Facebook page:
                ...
                if (!$currently_sycing) {
                  ...
                  Force Product Resync</a><p/>';
                } else {
                  echo '<p id="sync_status">' .
                  sprintf(__('<strong>Facebook product sync in progress, <br/>
                   please check back later and refresh the page.</strong></br>',
                    'facebook-for-woocommerce')) .
                  '</p>';
                }
              }
              ...
              if (!$currently_sycing) {
                echo '<p id="sync_status"></p><a href="#"
                  class="btn" onclick="facebookConfig()" id="set_dia">' .
                  esc_html($configure_button_text) .
                  '</a>';
